...
Simplified version: removes the per-scenario dimension, keeping:
- Comprehensive KPI dashboard
- SVG pie chart + radar chart
- Dimension analysis tables for risk category / injection technique / injection position / attack category
...
def generate_svg_pie_chart(
    ...
    if total == 0:
        msg = t["no_data"] if t else "No data"
    ...
    <div class="chart-center">
        <svg width="{size}" height="{size}" viewBox="0 0 {size} {size}">
            ...
            <text text-anchor="middle" dominant-baseline="middle">
                ...
                <tspan x="{center}" y="{center + 14}" style="font-size:12px;fill:var(--gray-400)">{t["pie_total"] if t else "Total"}</tspan>
            </text>
        </svg>
...
def generate_svg_radar_chart(
    ...
    n = len(items)
    if n < 3:
        msg = t["insufficient_dims"] if t else "Insufficient dimensions"
        return f'<p style="color: #94a3b8; text-align: center;">{msg}</p>'
    ...
    if "overall" in scores:
        ...
        center_text = (
            ...
            f'style="font-size:10px;fill:var(--gray-400)">{t["radar_avg"] if t else "Avg/5"}</text>'
        )
...
def _render_turn_tools(new_logs: list[dict], t: dict | None = None) -> str:
    ...
    title = t["tool_calls_title"] if t else "🔧 Tool Calls"
    return (
    ...
    for tr in turn_results:
        ...
        if turn_data and (turn_data.injection_in_user or turn_data.injection_in_tool):
            ...
            if turn_data.injection_in_user:
                parts.append(t["inj_user"] if t else "User message injection")
            if turn_data.injection_in_tool:
                parts.append(t["inj_tool"] if t else "Tool return injection")
        ...
        no_resp = t["no_response"] if t else "<No response>"
...
def _render_run_column(sr: SingleRunResult, run_idx: int, item, t: dict | None = None) -> str:
    ...
    _s = t["rubric_safety"] if t else "Safety"
    _a = t["rubric_adherence"] if t else "Adherence"
    _c = t["rubric_controllability"] if t else "Controllability"
    _avg = t["rubric_avg"] if t else "Avg"
    _gs = t["general_safety_eq"] if t else "General Safety"
    _trig = t["triggered"] if t else "Triggered"
    _conv = t["conv_label"] if t else "Conv"
    _eval = t["eval_label"] if t else "Eval"
    _reason = t["reason"] if t else "Reason"
...
def render_mt_result_card(result: MultiTurnResult, index: int, t: dict | None = None) -> str:
    ...
    _conv = t["conv_label"] if t else "Conv"
    _eval = t["eval_label"] if t else "Eval"
    _turns = t["turns_unit"] if t else "turns"
    _tc = t["tool_calls_unit"] if t else "tool calls"
    _runs = t["runs_unit"] if t else "runs"
    _reason_label = t["reason"] if t else "Reason"
    _no_reason = t["no_reason"] if t else "N/A"
    _confidence = t["confidence"] if t else "Confidence"
    _agg = t["aggregated_judgment"] if t else "Aggregated Judgment"
    ...
    if item.data_type == "harmful" and result.response_category:
        ...
        unknown = t["unknown"] if t else "❌ Unknown"
    ...
    _a = t["rubric_adherence"] if t else "Adherence"
    _c = t["rubric_controllability"] if t else "Controllability"
    _r = t["rubric_reliability"] if t else "Reliability"
    _avg = t["rubric_avg"] if t else "Avg"
    if item.data_type == "harmful":
        cat_name = rc_labels.get(result.response_category, t["unknown"] if t else "Unknown") if result.response_category else (t["unknown"] if t else "Unknown")
        _rt = t["risk_trigger"] if t else "Risk Trigger"
        judgment_details = f"{_rt}: {cat_name}"
        if result.general_safety_score > 0:
            _gs = t["general_safety_colon"] if t else "General Safety"
            judgment_details += f" | {_gs}: {result.general_safety_score}/5"
        if result.triggered_risks:
            _tr = t["triggered_risks"] if t else "Triggered Risks"
            judgment_details += f" | {_tr}: {html.escape(', '.join(result.triggered_risks[:5]))}"
        if result.rubric and result.rubric.average > 0:
            rb = result.rubric
            _scores = t["rubric_scores"] if t else "Scores"
            rubric_inline = (
        ...
        if result.num_runs > 1:
            _nr = t["num_runs_label"] if t else "Runs"
...
def render_group_section(
    ...
    _prefix = t["group_prefix"] if t else "📦 Group"
    _suffix = t["injections_suffix"] if t else "injections"
...
def render_dimension_tables(results: list[MultiTurnResult], t: dict | None = None) -> str:
    injected = [r for r in results if r.item.data_type == "harmful"]
    sections = []
    _count = t["th_count"] if t else "Count"
    _refusal = t["th_refusal_rate"] if t else "Refusal Rate"
    ...
    if injected:
        ...
        _title = t["dim_risk_subcategory"] if t else "🔒 By Risk Category — Subcategory"
        _th = t["th_risk_subcategory"] if t else "Risk Category / Subcategory"
        sections.append(f'''
    ...
    if injected:
        ...
        if by_risk_tech:
            ...
            _title2 = t["dim_risk_technique"] if t else "💉 By Risk Category — Injection Technique"
            _th2 = t["th_risk_technique"] if t else "Risk Category / Injection Technique"
            sections.append(f'''
            <div class="dim-section">
                <h3>{_title2}</h3>
                <table class="dim-table dim-table-tree">
    ...
    if injected:
        ...
        if by_pos:
            rows = ""
            pos_labels = {
                "user": t["pos_user"] if t else "User Message",
                "tool": t["pos_tool"] if t else "Tool Return",
                "both": t["pos_both"] if t else "Both",
                "none": t["pos_none"] if t else "None",
            }
            ...
            _title3 = t["dim_injection_position"] if t else "📍 By Injection Position"
            _th3 = t["th_injection_position"] if t else "Injection Position"
            sections.append(f'''
            <div class="dim-section">
                <h3>{_title3}</h3>
    ...
    if injected:
        ...
        _title4 = t["dim_attack_category"] if t else "🏷️ By Attack Category"
        _th4 = t["th_category"] if t else "Category"
        sections.append(f'''
        <div class="dim-section">
            <h3>{_title4}</h3>
            <table class="dim-table">
