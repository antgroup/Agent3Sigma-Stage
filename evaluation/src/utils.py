...
Contains common functions for data loading, JSON parsing, session log reading, etc.
Utility helpers for data loading, JSON parsing, and session-log reading.
"""
...
def read_session_total_tokens(session_id: str, docker: Optional[str] = None, docker_home: str = "/home/node") -> int:
    ...
    # Some providers do not emit reliable token-usage fields in their logs;
    # list any such provider names here to fall back to a tiktoken estimate.
    providers_without_usage: set = set()
    if (provider or "").lower() in providers_without_usage:
